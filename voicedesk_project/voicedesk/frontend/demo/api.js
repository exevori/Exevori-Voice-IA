...
export function createDemoApi({ storage, isSignedIn = () => false, origin = "http://127.0.0.1:3000" }) {
  ...
  const api = async (input, options = {}) => {
    ...
    if (path === "/dashboard/activity" && read) return result({ activities: db.calls.map(c => ({ id: c.id, type: "call_inbound", title: c.intent, description: c.ai_summary, occurred_at: c.created_at, created_at: c.created_at, link: "/calls" })) });
